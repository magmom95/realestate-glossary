  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-12">
      ...
      <section className="text-center mb-16">
        <p className="text-xs font-bold text-[#e8c97d] tracking-[3px] uppercase mb-4">
          검색 기반 학습 엔진
        </p>
        <h1 className="font-display text-5xl md:text-7xl font-normal tracking-tight leading-[1.05] mb-5">
          부동산 용어,<br />
          <em className="text-[#e8c97d] not-italic">비유로 이해하세요</em>
        </h1>
        <p className="text-base text-[#8a8276] leading-relaxed max-w-md mx-auto mb-10">
          ...
          {ALL_CATEGORIES.map((cat) => (
            <Link key={cat.value}
              ...
              className="flex flex-col items-center gap-2 py-5 px-3
                ...
                text-xs font-medium text-[#8a8276]
      ...
      {/* Beginner Flow */}
      <section className="mb-16">
        <h2 className="text-lg font-bold mb-1.5 tracking-tight">🌱 입문자 학습 흐름</h2>
        <p className="text-sm text-[#8a8276] mb-5">검색이 아닌 &quot;학습 흐름&quot;으로 이어서 공부하세요</p>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          {BEGINNER_FLOWS.map((flow) => (
            <div key={flow.title}
              className="bg-[#141414] border border-white/[0.07] rounded-xl p-5">
              <p className="text-sm font-bold text-[#e8c97d] mb-3">{flow.title}</p>
